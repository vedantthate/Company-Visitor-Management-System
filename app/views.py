...
def Managevisiter(request):
    ...
    if request.method == 'POST':
        ...
        try:
            name = Add_new_visiter.objects.filter(fullname__icontains=Searchfield).order_by('fullname')
            number = Add_new_visiter.objects.filter(phonenum__icontains=Searchfield).order_by('fullname')
            
            manage_search = name | number
            if not manage_search:
                raise RegisterError("No data found...!")
            
            visitors = manage_search.distinct()
    ...
    if not visitors:
        try:    
            visitors = Add_new_visiter.objects.all().order_by('fullname')
        except :
...
def Visitbydate(request):
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            visitors = Add_new_visiter.objects.filter(date__range=[Frodate,Todate]).order_by('fullname')
